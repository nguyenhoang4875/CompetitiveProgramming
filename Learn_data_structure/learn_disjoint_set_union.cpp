...
struct Dsu {
  ...
  void unionSet(int a, int b)  {
    ...
    b = findSet(b);
    if(a != b) {
        if(sz[a] < sz[b]) swap(a, b); // set a is root of the bigger size of tree
        parent[b] = a;
        sz[a] += sz[b]; // update the new size of new the merge tree 
    }
  }
};
...
struct DsuMinSum {
    vector<int> parent, sz, mn, sum;
    DsuMinSum() {};
    DsuMinSum(int n) : parent(n), sz(n), mn(n), sum(n) {};
