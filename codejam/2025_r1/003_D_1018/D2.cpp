...
void solve() {
    ...
    For(i, 1, n) {
        ps[i] = ps[i - 1] + a[i - 1].S;
        psm[i] = psm[i - 1] + abs(a[i - 1].F) * a[i - 1].S;
    }
    debug(ps);
    debug(psm);
    ...
    auto getAns = [](int s, int b, int c) -> int {
        debug(s, b, c, s * s + b * s + c);
        return s * s + b * s + c;
    };

    // move to not a point
    debug("move not a point");
    For(i, 0, n) {
        int psl = ps[i];
        int psr = ps[n] - ps[i];
        ...
        auto cal = [&](int b, int c) {
            ...
            if (ct < ll) {
                ...
                for(int s = s1 - 5; s < s1 + 5; ++s) {
                    ...
                    // int val = (ll <= s and s <= rr) ? getAns(s, b, c) : oo;
                    // res = min(res, val);
                }
            }
            return res;
