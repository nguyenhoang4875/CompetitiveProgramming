...
void solve() {
    ...
    vi treeSize(n + 1);            // treeSize[u]: size of subtree at u

    function<void(int, int)> dfs = [&](int u, int par) {
        vi cur(n + 1, -oo), pre(n + 1, -oo);
        cur[0] = 0;
        treeSize[u] = 0;
        for (int &v : adj[u]) {
            if (v == par) continue;
            ...
            for (int i = 0; i <= treeSize[u]; i++) {
                for (int j = 0; j <= treeSize[v]; j++) {
                    cur[i + j] = max(cur[i + j], pre[i] + f[v][j]);
                }
            }
            treeSize[u] += treeSize[v];
        }
        Rep(i, treeSize[u] + 1) f[u][i + 1] = a[u] + cur[i];
        f[u][0] = 0;
        treeSize[u]++;
    };
    ...
    for (int k = 1; k <= n; k++) {
        ...
        for (int root = 1; root <= n; root++) {
            if (treeSize[root] >= k) ans = max(ans, f[root][k]);
        }
        cout << ans << " ";
    }
    cout << el;
