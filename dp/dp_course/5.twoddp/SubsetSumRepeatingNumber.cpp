...
// a[......idx]
int subsetSumRepeatingNumbers(int idx, int sum) {
    // base case
    // no element left for sum
    if (idx == 0) return sum == 0;
    if(memo[idx][sum] != -1) return memo[idx][sum];
    bool ans = false;
    // include a[idx] in sum multiple times
    if (sum >= a[idx]) {
        ans |= subsetSumRepeatingNumbers(idx, sum - a[idx]);
    }

    // exclude a[idx]
    ans |= subsetSumRepeatingNumbers(idx - 1, sum);
...
void solve() {
    ...
    cout << subsetSumRepeatingNumbers(n, sum) << '\n';
}
