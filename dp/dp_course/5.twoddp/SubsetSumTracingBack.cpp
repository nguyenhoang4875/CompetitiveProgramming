...
void printSubsetSum(int idx, int sum) {
    if (idx == 0) return;

    if (subsetSum(idx - 1, sum - a[idx])) {
        cout << a[idx] << " ";
        printSubsetSum(idx - 1, sum - a[idx]);
    } else if (subsetSum(idx - 1, sum)) {
        printSubsetSum(idx - 1, sum);
    }
}
