...
using namespace std;
/*
Give N meeting room, each room have: cost, square, distance
Give Q queries each query contains: low, high, r
For each query find min cost with: low <= square <= high, distance <= r
Constraints:
    1 <= N <= 10^5
    1 <= low, high <= 10^9
    1 <= distance, r <= 10^14
...
void solve() {
    ...
    vector<Node> vn(n + 1);
    For(i, 1, n) { cin >> vn[i].sq >> vn[i].cost >> vn[i].dist; }
